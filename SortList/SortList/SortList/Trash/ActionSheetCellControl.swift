...
class ActionSheetCellControl: UIAlertController {
	...
	func ButtonActionSheetCellItems(toDoItem: ToDoItem?,
	                                itLabel: UILabel) -> UIAlertController {// (title: String?)

		let title       = toDoItem!.item! as String//itLabel.text
		let itemChecked = toDoItem!.checked

		let myActionSheet = UIAlertController(title: title,
		                                      message: nil,
	...
	func chaingeTextNarrovIfCheckedOrNot (needCheck: Bool, itemLabel: UILabel) -> Bool {
		...
		if needCheck {
			firstAttributes = [NSForegroundColorAttributeName: UIColor.lightGrayColor(),
			                   NSStrikethroughStyleAttributeName: 1]
		} else {
			firstAttributes = [NSForegroundColorAttributeName: itemLabel.tintColor,
			                   NSStrikethroughStyleAttributeName: 0]
		}
		attributedString.addAttributes((firstAttributes as? [String: AnyObject])!,
		                               range: string.rangeOfString(itemName!))
