...
class DetailViewController: UIViewController,
	...
	@IBAction func detailViewDoneButton(sender: AnyObject) {
		...
		if receivedCell == nil {
			//new value
			ToDoItem.insertToDoItemWithItem(detailViewTextInputEdit.text,
			                                checked: false,
			                                createdDate: NSDate.getCurrentDate(),
			                                colorItem: selectedColor,
			                                context: moc)

		} else {
			ToDoItem.rewriteToDoItemWithParameters(&receivedCell,
			                                       item: detailViewTextInputEdit.text,
			                                       checked: nil,
			                                       colorItem: selectedColor)
		}
	...
	func getColorOfItemsFromreceivedCell() -> UIColor {
		guard receivedCell != nil && receivedCell!.colorItem != nil else {
			...
			return ToDoItem.getDefaultColorFortem()
		}
	...
	private func showColorPicker() {
		...
		colorPickerVc.preferredContentSize = CGSize(width: 302, height: 302)
