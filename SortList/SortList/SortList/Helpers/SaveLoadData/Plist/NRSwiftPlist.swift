...
func plistGet(key: String,
              forPlistNamed: String,
              needToSaveBetweenSessions: Bool = true) throws -> AnyObject? {

	//if let path:String = NSBundle.mainBundle().pathForResource(forPlistNamed, ofType: "plist"){
	if let path: String = getPathToSaveData(forPlistNamed,
...
func plistSet(newValue: AnyObject,
              forKey: String,
              inPlistNamed: String,
              needToSaveBetweenSessions: Bool = true) throws {

	// if let path: NSString = NSBundle.mainBundle().pathForResource(inPlistNamed, ofType: "plist") {
	if let path: String = getPathToSaveData(inPlistNamed,
	                                        needToSaveBetweenSessions: needToSaveBetweenSessions) {
...
private func getPathToSaveData (inPlistNamed: String,
...
private func getPlistKeyValue(key: String, path: String) throws -> AnyObject? {
	...
	if let plistData = NSData(contentsOfFile: path) {
		...
		do {
			let plist = try (NSPropertyListSerialization.propertyListWithData(plistData,
				options: .Immutable,
				format: formats) as? NSDictionary)!
	...
	if let plistData = NSData(contentsOfFile: path as String) {
		...
		do {
			let plist = try (NSPropertyListSerialization.propertyListWithData(plistData,
																																				...
																																				format: formats) as? NSMutableDictionary)!
			if path == "" {
