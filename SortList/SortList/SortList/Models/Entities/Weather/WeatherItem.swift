...
class WeatherItem: NSManagedObject {
	...
	static func insertNewObjectIntoContext(context: NSManagedObjectContext) -> NSManagedObject? {
		return NSEntityDescription.insertNewObjectForEntityForName(entityName as String,
		                                                           inManagedObjectContext: context)
	}

	static func insertWeatheItemWithItem(structOfItems: WeatherItemStructure,
		...
		weatherItem.mainTempMin         = structOfItems.mainTempMin
		weatherItem.mainTempMax         = structOfItems.mainTempMax
	...
	static func allWatherItems() -> [WeatherItem]? {
		...
		let array = CoreDataUtil.fetchEntity(entityName as String,
		                                     predicate: nil,
		                                     sortDescriptors: [nameSort]) as? [WeatherItem]
	...
	static func getElementDictOfClass () -> [String: String] {
		...
		Dict.updateValue("Minimum temperature at the moment", forKey: "mainTempMin")
		Dict.updateValue("Maximum temperature at the moment", forKey: "mainTempMax")
	...
	subscript(name: String) -> String? {
		...
		get {
			...
			if let _ = WeatherItem.elementDictOfClass[name] {
				...
				case "mainTempMin":         valueToReturn = self.mainTempMin.getStringFormat (2)
				case "mainTempMax":         valueToReturn = self.mainTempMax.getStringFormat (2)
